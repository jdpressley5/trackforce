...
//			}
	...
	public Response updateAssociates(@HeaderParam("Authorization") String token,
		...
		for (int associateId : ids) {
			...
			if (clientId != 0) toBeUpdated.setClient(clientService.getClient(clientId));
			if (isApproved >= 0) toBeUpdated.getUser().setIsApproved(isApproved);
	...
	@GET
	@Path("/nass/")
	public Response getNAssociates() { return Response.status(200).entity(associateService.getNAssociates()).build(); }
}