...
/** <p>Class that provides RESTful services for the batch listing and batch details page.</p>
 * @version v6.18.06.13 */
@Path("/batches")
@Api(value = "batches")
...
public class BatchResource
{
	...
	@GET
	@ApiOperation(value = "Returns all Batches",
			notes = "Returns a list of a list of all batches optionally filtered by start and end dates.")
			...
			@HeaderParam("Authorization") String token)
	{
		...
		Set<Integer> authorizedRoles = new HashSet<>(Arrays.asList(1, 2, 3, 4));
			...
			if (startDate != null && endDate != null) {
				...
				while(i < batches.size()) {
					if(batches.get(i).getStartDate().before(start)) {
						...
						batches.remove(i);
					} else { //increments ONLY if no elements were removed
						i++;
					}
				}
			}
	 ...
	 * @version v6.18.06.13 */
	@GET
	@ApiOperation(value = "Returns associates for batch",
			notes = "Returns list of associates for a specific batch based on batch id.")
	@Path("{id}/associates")
	public Response getBatchAssociates(@PathParam("id") Integer id, @HeaderParam("Authorization") String token) {
		...
		if (authorizedRoles.contains(role))// results and status set in here
		{
	...
	/*	Set<Integer> authorizedRoles = new HashSet<>(Arrays.asList(new Integer[] { 1, 2, 3, 4}));
		...
		List<TfBatch> batches = bd.getBatchesForPredictions(courseName,
				new Timestamp(startDate), new Timestamp(endDate));
	...
	private Integer getUnmappedCount(Set<TfAssociate> associates) {
		int n = 0;
	...
	@GET
	@ApiOperation(value = "Returns batch information given a batch id",
			notes = "Returns batch information based on batch id.")
	@Path("/batch/{id}")
