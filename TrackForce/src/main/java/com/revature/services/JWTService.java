...
{	
	...
	public Boolean validateToken(String token) {
		...
		if (token != null) claims = processToken(token);
		if (claims != null) tokenUsername = claims.getSubject();
		if (tokenUsername != null) tfUser = userService.getUser(tokenUsername);
		// makes sure the token is fresh and usernames are equal
		if (tfUser != null) verified = (!isTokenExpired(token) && tfUser.getUsername().equals(tokenUsername));
		return verified;
	}
	...
	public static Claims processToken(String token) {
		...
		try {
			if (token == null) throw new UnsupportedJwtException("token null");
	...
	private static Date generateExpirationDate() { return new Date(System.currentTimeMillis() + EXPIRATION * 1000 * 60); }
